			       ...
			       struct dsa_db db)
{
	...
	if (a) {
		...
		//trace_dsa_mdb_add_bump(dp, mdb->addr, mdb->vid, &db, &a->refcount);
		goto out;
	}
			       ...
			       struct dsa_db db)
{
	...
	if (!refcount_dec_and_test(&a->refcount)) {
		//trace_dsa_mdb_del_drop(dp, mdb->addr, mdb->vid, &db,&a->refcount);
		goto out;
	}
				     ...
				     struct dsa_db db)
{
	...
	if (a) {
		refcount_inc(&a->refcount);
		//trace_dsa_lag_fdb_add_bump(lag->dev, addr, vid, &db,&a->refcount);
		goto out;
	}
				     ...
				     struct dsa_db db)
{
	...
	if (!refcount_dec_and_test(&a->refcount)) {
		//trace_dsa_lag_fdb_del_drop(lag->dev, addr, vid, &db,&a->refcount);
		goto out;
	}
